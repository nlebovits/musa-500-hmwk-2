...
Secondly, we regress residuals on the nearest neighbor residuals, thereby filtering the spatial information out of the OLS residuals and decomposing the residuals $ε$ into two parts, one with a spatial pattern $λWε$ and one which is simply random noise $u$. The part with a spatial pattern can be thought of as some variable with a spatial component missing from the first OLS regression.
...
```{r spatial error}
spatial_error_reg = errorsarlm(formula = LNMEDINC ~ LNMEDHVAL + PCTVACANT, 
                               data = reg_data, 
                               nb2listw(reg_data$nb))

summary(spatial_lag_reg)

# NOTE: I AM GETTTING SLIGHTLY DIFFERENT VALUES THAN EUGENE HERE. COULD IT BE THE RESULT OF RANDOMNESS?
```
As with our OLS before, we will calculate the likelihood ratio for our spatial lag model.
...
reg_data = reg_data |>
              mutate(
                spatial_error_resids = errorsarlm(formula = LNMEDINC ~ LNMEDHVAL + PCTVACANT, data = reg_data, nb2listw(reg_data$nb))$residuals
              )
...
adapt_bandwidth_est = GWmodel::bw.gwr(formula = LNMEDINC ~ LNMEDHVAL + PCTVACANT, 
                                data = reg_data |> sf::as_Spatial(),
                                approach = 'AIC', 
                                kernel = 'gaussian', 
                                adaptive = TRUE)

adapt_bandwidth_est = gwr.sel(formula=LNMEDINC~LNMEDHVAL+PCTVACANT, 
                              data = reg_data |> sf::as_Spatial(),
                              method = "aic",
                              adapt = TRUE)
```
...
fixed_bandwidth_est = gwr.sel(formula=LNMEDINC~LNMEDHVAL+PCTVACANT, 
                              data = reg_data |> sf::as_Spatial(),
                              method = "aic",
                              adapt = FALSE)
...
```{r adaptive bw gwr}
gwrmodel_adaptive = gwr(formula = LNMEDINC ~ LNMEDHVAL + PCTVACANT,
                        data = reg_data |> sf::as_Spatial(),
...
```{r fixed bw gwr}
# Perform a basic GWR
gwrmodel_fixed = gwr(formula = LNMEDINC ~ LNMEDHVAL + PCTVACANT,
              data = reg_data |> sf::as_Spatial(),
...
reg_data = reg_data |>
              mutate(
                coef_lnmedhval_adapt_stand = gwr_adaptive_results$LNMEDHVAL / gwr_adaptive_results$LNMEDHVAL_se,
                ...
                coef_lnmedhval_fixed_stand = gwr_fixed_results$LNMEDHVAL / gwr_fixed_results$LNMEDHVAL_se,
                coef_pctvacant_fixed_stand = gwr_fixed_results$PCTVACANT / gwr_fixed_results$PCTVACANT_se,
                gwr_fixed_resids = gwr_fixed_results$gwr.e,
                gwr_fixed_r2 = gwr_fixed_results$localR2,
              )

coef_lnmedhval_adapt_stand_map = tm_shape(reg_data)+
                                    tm_fill(col='coef_lnmedhval_adapt_stand', 
                                            breaks=c(-Inf, -6, -4, -2, 0, 2, 4, 6, Inf), 
                                            title='Standardized coefficient of LNMEDHVAL', 
                                            palette ='-RdBu')+
                                    tm_layout(frame=FALSE, 
                                              main.title = 'Median House Value (Log)',
                                              title = "Adaptive GWR")
...
tmap_arrange(coef_lnmedhval_adapt_stand_map, coef_pctvacant_adapt_stand_map, ncol=2)
```
...
gwr_coef_lnmedinc = tm_shape(reg_data)+
                        tm_fill(col = 'coef_lnmedhval_adapt_stand', 
                                style = 'quantile', 
                                title = 'GWR Coef., LNMEDINC', 
                                palette = 'Blues')+
                        tm_layout(frame = FALSE)
